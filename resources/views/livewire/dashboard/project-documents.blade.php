        ...
        <div class="space-y-4">
            <div class="grid sm:grid-cols-2 gap-4">
                @foreach ($step->requiredDocuments as $document)
                    ...
                    @endphp
                    <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-4">
                                ...
                                <div class="min-w-0 flex-grow">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @if($document->is_required)
                                            <span class="inline-flex px-1.5 py-0.5 rounded-md text-xs font-medium bg-red-50 text-red-600">
                                                Required
                                            </span>
                                        @endif
                                    </div>
                                ...
                                @if ($submittedDoc)
                                    ...
                                    <div class="flex items-center gap-1 md:gap-2">
                                        @if($submittedDoc->rejection_reason)
                                            <button 
                                                x-data="" 
                                                x-tooltip.raw="{{ $submittedDoc->rejection_reason }}"
                                        ...
                                        @if($submittedDoc->file_path)
                                            <button 
                                                wire:click="viewDocument({{ $submittedDoc->id }})"
        ...
        <x-filament::modal id="preview-documents" width="7xl">
            <div class="flex items-center justify-between w-full gap-4">
                ...
                <div class="flex items-center gap-4 min-w-0">
                    <!-- Icon -->
                    <div class="flex-shrink-0 w-12 h-12 rounded-xl bg-primary-50 flex items-center justify-center ring-1 ring-primary-100">
                        @if($fileType === 'pdf')
            ...
            <div class="relative p-6">
                @if($previewUrl)
                    <div class="rounded-xl overflow-hidden bg-gray-50 ring-1 ring-gray-200">
                        ...
                        @elseif(in_array($fileType, ['jpg', 'jpeg', 'png', 'gif']))
                            <div class="relative aspect-video flex items-center justify-center bg-gray-50">
                                <img src="{{ $previewUrl }}" alt="Document Preview" class="max-w-full max-h-[calc(100vh-16rem)] object-contain rounded-lg shadow-sm">
                            </div>
                        @else
                    ...
                    @if($totalDocuments > 1)
                        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-gray-900/80 rounded-full p-2 backdrop-blur-sm">
