    ...
    <div class="hidden lg:block">
        <div class="grid grid-cols-12 gap-4 items-center">
            ...
            <div class="col-span-2">
                ...
                <div class="flex items-center gap-3">
                    <div class="flex -space-x-2">
                        ...
                        <div class="w-8 h-8 bg-gradient-to-br from-primary-400 to-primary-600 dark:from-primary-500 dark:to-primary-700 text-white rounded-full flex items-center justify-center text-xs font-bold border-2 border-white dark:border-gray-800 shadow-sm hover:scale-110 transition-transform duration-200 cursor-pointer"
                        ...
                        <div class="w-8 h-8 bg-gray-400 dark:bg-gray-600 text-white rounded-full flex items-center justify-center text-xs font-bold border-2 border-white dark:border-gray-800 shadow-sm hover:scale-110 transition-transform duration-200 cursor-pointer"
                    ...
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">{{ $task->assignedUsers->first()->name }}</span>
                    @else
                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ $task->assignedUsers->count() }} people</span>
                    @endif
                </div>
                @else
                <div class="flex items-center gap-3 text-gray-400 dark:text-gray-500">
                    <div class="w-8 h-8 bg-gray-100 dark:bg-gray-700 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors cursor-pointer">
            ...
            <div class="col-span-1">
                <div class="flex justify-center">
                    @if($task->project)
                    <div class="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 text-indigo-800 dark:text-indigo-300 rounded-lg text-xs font-semibold max-w-full hover:from-indigo-200 hover:to-purple-200 dark:hover:from-indigo-900/50 dark:hover:to-purple-900/50 transition-all duration-200 border border-indigo-200 dark:border-indigo-700 shadow-sm"
                        title="{{ $task->project->name }}">
                        <x-heroicon-o-folder class="w-3 h-3 flex-shrink-0" />
                        <span class="truncate">{{ Str::limit($task->project->name, 8) }}</span>
                    </div>
                    @else
                    <div class="flex items-center gap-1.5 px-3 py-2 bg-gray-100 dark:bg-gray-700 border-2 border-dashed border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 rounded-lg text-xs hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors cursor-pointer"
            ...
            <div class="col-span-1">
                <div class="flex flex-col items-start space-y-1">
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <div class="p-1 rounded {{ $isOverdue ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : ($isToday ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400' : 'text-gray-600 dark:text-gray-400') }}">
                            <x-heroicon-o-calendar-days class="w-4 h-4" />
                        </div>

                        <div class="flex flex-col">
                            <span class="font-medium {{ $isOverdue ? 'text-red-600 dark:text-red-400' : ($isToday ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-700 dark:text-gray-300') }}">
                                @if($isToday)
                                Today
                                @elseif($isTomorrow)
                                Tomorrow
            ...
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-start gap-3 flex-1">
                    ...
                    <button wire:click="toggleTaskCompletion"
                        ...
                        @else
                        <div class="w-6 h-6 rounded-full border-2 border-gray-300 dark:border-gray-600 hover:border-primary-500 dark:hover:border-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-all duration-200 flex items-center justify-center">
                            <div class="w-0 h-0 bg-primary-500 dark:bg-primary-400 rounded-full hover:w-3 hover:h-3 transition-all duration-200"></div>
                        </div>
                        @endif
                    </button>
                    ...
                    <div class="flex-1 min-w-0">
                        <h4 class="font-semibold text-gray-900 dark:text-gray-100 text-base leading-snug {{ $task->status === 'completed' ? 'line-through text-gray-500 dark:text-gray-400' : '' }} hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                            {{ $task->title }}
                        </h4>
                        @if($task->description)
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if($task->assignedUsers->count() === 1)
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300 truncate max-w-24">{{ $task->assignedUsers->first()->name }}</span>
                        @else
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $task->assignedUsers->count() }} people</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-2 text-gray-400 dark:text-gray-500">
                        <div class="w-6 h-6 bg-gray-100 dark:bg-gray-700 border border-dashed border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center">
