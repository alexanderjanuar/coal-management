...
<div class="space-y-4" x-data="{
        ...
        <div class="flex-1 order-1">
            <div class="relative group">
                <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400
                            group-focus-within:text-blue-500 transition-colors duration-200" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <input type="text" wire:model.live.debounce.500ms="filterData.search"
                    placeholder="Cari berdasarkan nama task, deskripsi, atau proyek..." class="w-full pl-10 pr-10 py-3 border border-gray-300 dark:border-gray-600 rounded-xl
                ...
                <div class="absolute right-3 top-1/2 transform -translate-y-1/2">
                    @if(!empty($filterData['search']))
                        <button wire:click="$set('filterData.search', '')" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300
        ...
        <div class="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 flex-shrink-0 order-2">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="hidden md:flex items-center bg-gradient-to-br from-gray-100 to-gray-50
                    ...
                    <button type="button" @click="toggleViewMode('kanban')"
    ...
    : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500' }}">
            ...
            <div class="flex items-center gap-2 px-2 sm:px-3 py-1.5 sm:py-2
                ...
                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <span class="hidden sm:inline">{{ number_format($totalTasks) }}
                        task{{ $totalTasks !== 1 ? 's' : '' }}</span>
    ...
    <div class="md:hidden" x-data="{ isMobileToggleVisible: true }">
        <div class="flex items-center justify-center" x-show="isMobileToggleVisible"
            ...
            <div class="flex items-center bg-gradient-to-br from-gray-100 to-gray-50
                ...
                <button type="button" @click="toggleViewMode('kanban')" class="relative flex-1 flex items-center justify-center gap-2 px-3 py-2.5
    ...
    : 'text-gray-600 dark:text-gray-400' }}">
                ...
                <button type="button" @click="toggleViewMode('list')" class="relative flex-1 flex items-center justify-center gap-2 px-3 py-2.5
                           rounded-lg text-sm font-medium transition-all duration-300 ease-out
                           {{ ($filterData['view_mode'] ?? 'kanban') === 'list'
    ? 'bg-white dark:bg-gray-600 text-blue-600 dark:text-blue-400 shadow-lg scale-105'
    : 'text-gray-600 dark:text-gray-400' }}">
                    <svg class="w-4 h-4 transition-transform duration-300
                                {{ ($filterData['view_mode'] ?? 'kanban') === 'list' ? 'scale-110' : '' }}" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                    </svg>
    ...
    <div x-show="showAdvancedFilters" x-transition:enter="transition ease-out duration-300 transform"
        x-transition:enter-start="opacity-0 scale-95 -translate-y-4"
        x-transition:enter-end="opacity-100 scale-100 translate-y-0"
        x-transition:leave="transition ease-in duration-200 transform"
        x-transition:leave-start="opacity-100 scale-100 translate-y-0"
        x-transition:leave-end="opacity-0 scale-95 -translate-y-4" class="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700
        ...
        <div class="p-4 sm:p-5">
            ...
            <div class="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="showAdvancedFilters = false" class="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100
            ...
            <div class="flex items-center justify-between pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center gap-2">
                    <button wire:click="resetFilters" class="group flex items-center gap-2 px-4 py-2 text-sm font-medium
                    ...
                    <button @click="showAdvancedFilters = false" class="flex items-center gap-2 px-5 py-2
