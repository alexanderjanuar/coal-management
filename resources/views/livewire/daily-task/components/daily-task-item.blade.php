...
<div x-data="taskItemManager({{ $task->id }})" class="space-y-2" x-init="init()">
    <div class="">
        ...
        <div
            ...
            <div class="min-w-[1200px]">
                <div class="grid grid-cols-12 gap-2 lg:gap-4 items-center h-12">

                    {{-- Checkbox & Completion Toggle --}}
                    <div class="col-span-1 flex items-center justify-center h-full">
                        <button wire:click="toggleTaskCompletion"
                            class="flex-shrink-0 hover:scale-110 transition-transform duration-200 w-6 h-6 flex items-center justify-center"
                    ...
                    <div class="col-span-4 h-full flex items-center">
                        <div class="flex-1 cursor-pointer" wire:click="viewDetails">
                            <h2
                                class="font-semibold text-gray-900 dark:text-gray-100 text-sm lg:text-base leading-tight {{ $task->status === 'completed' ? 'line-through text-gray-500 dark:text-gray-400' : '' }} hover:text-primary-600 dark:hover:text-primary-400 transition-colors truncate">
                                {{ $task->title }}
                            </h2>
                            @if($task->description)
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1 truncate">
                    ...
                    <div class="col-span-2 h-full flex items-center">
                        <div class="relative w-full" x-data="{ statusOpen: false, buttonRect: {} }">
                            <button @click="statusOpen = !statusOpen; buttonRect = $el.getBoundingClientRect()" class="inline-flex items-center gap-2 px-3 py-2 h-8 rounded-lg text-xs font-semibold transition-all duration-200 hover:scale-105 w-full justify-center shadow-sm border
                        {{ match($task->status) {
                        ...
                        {{ match($task->status) {
                            ...
                            <template x-teleport="body">
                                <div x-show="statusOpen" x-cloak @click.away="statusOpen = false"
                                    ...
                                    class="fixed w-44 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 py-2 overflow-hidden z-50"
                                    x-bind:style="{
                    ...
                    <div class="col-span-1 h-full flex items-center justify-center">
                        <div class="relative" x-data="{ priorityOpen: false, buttonRect: {} }">
                            <button @click="priorityOpen = !priorityOpen; buttonRect = $el.getBoundingClientRect()"
                                class="inline-flex items-center gap-1.5 px-3 py-2 h-8 rounded-lg text-xs font-bold transition-all duration-200 hover:scale-105 shadow-sm border justify-center w-full
                            {{ match($task->priority) {
                    ...
                    <div class="col-span-2 h-full flex items-center">
                        <div class="relative w-full" x-data="{ assigneeOpen: false, buttonRect: {} }">
                            <button @click="assigneeOpen = !assigneeOpen; buttonRect = $el.getBoundingClientRect()"
                                class="w-full flex items-center gap-2 px-3 py-2 h-8 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-600 transition-all duration-200">
                                ...
                                <div class="flex items-center gap-2 flex-1 min-w-0">
                                    <div class="flex -space-x-1">
                                        ...
                                        <div
                                            class="w-6 h-6 bg-gray-400 dark:bg-gray-600 text-white rounded-full flex items-center justify-center text-xs font-bold border-2 border-white dark:border-gray-800 shadow-sm">
                                    ...
                                    <span
                                        class="hidden xl:inline text-sm font-medium text-gray-700 dark:text-gray-300 truncate">
                                ...
                                <div class="flex items-center gap-2 text-gray-400 dark:text-gray-500 flex-1">
                                    <div
                                        class="w-6 h-6 bg-gray-100 dark:bg-gray-700 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center">
                                        <x-heroicon-o-plus class="w-3 h-3" />
                                    </div>
                                    <span class="hidden lg:inline text-sm truncate">Unassigned</span>
                                </div>
                                @endif
                                <x-heroicon-o-chevron-down
                            ...
                            <template x-teleport="body">
                                <div x-show="assigneeOpen" x-cloak @click.away="assigneeOpen = false"
                                    ...
                                    x-transition:leave-start="opacity-100 scale-100"
                                    x-transition:leave-end="opacity-0 scale-95"
                                    class="fixed w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 py-2 overflow-hidden max-h-80 overflow-y-auto z-50"
                                    x-bind:style="{
                                    top: (buttonRect.bottom + window.scrollY + 8) + 'px',
                                    ...
                                    <button
                                        wire:click="{{ $isAssigned ? 'unassignUser' : 'assignUser' }}({{ $userId }})"
                    ...
                    <div class="col-span-1 h-full flex items-center justify-center">
                        <div class="relative" x-data="{ projectOpen: false, buttonRect: {} }">
                            <button @click="projectOpen = !projectOpen; buttonRect = $el.getBoundingClientRect()" class="flex items-center gap-1.5 px-3 py-2 h-8 rounded-lg text-xs font-semibold transition-all duration-200 hover:scale-105 border shadow-sm w-full justify-center
                            @if($task->project)
                            ...
                            <template x-teleport="body">
                                <div x-show="projectOpen" x-cloak @click.away="projectOpen = false"
                                    ...
                                    x-transition:leave-start="opacity-100 scale-100"
                                    x-transition:leave-end="opacity-0 scale-95"
                                    class="fixed w-80 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 overflow-hidden max-h-96 overflow-y-auto z-50"
                                    x-bind:style="{
                                    top: (buttonRect.bottom + window.scrollY + 8) + 'px',
                                    ...
                                    <div class="py-2">
                                        ...
                                        <button wire:click="updateProject({{ $projectId }})"
                    ...
                    <div class="col-span-1 h-full flex items-center justify-center">
                        <div class="relative" x-data="{ dateOpen: false, buttonRect: {} }">
                            ...
                            <button @click="dateOpen = !dateOpen; buttonRect = $el.getBoundingClientRect()" class="flex items-center gap-1.5 px-3 py-2 h-8 rounded-lg transition-all duration-200 hover:scale-105 border shadow-sm w-full justify-center
                            {{ $isOverdue ? 
                                ...
                                <div
                                    class="p-0.5 rounded flex-shrink-0 {{ $isOverdue ? 'bg-red-200 dark:bg-red-800' : ($isToday ? 'bg-yellow-200 dark:bg-yellow-800' : 'bg-gray-200 dark:bg-gray-600') }}">
                            ...
                            <template x-teleport="body">
                                <div x-show="dateOpen" x-cloak @click.away="dateOpen = false"
                                    ...
                                    x-transition:leave-start="opacity-100 scale-100"
                                    x-transition:leave-end="opacity-0 scale-95"
                                    class="fixed w-80 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 z-50"
                                    x-bind:style="{
                                    top: (buttonRect.bottom + window.scrollY + 8) + 'px',
                                    left: Math.max(8, Math.min(buttonRect.left + window.scrollX, window.innerWidth - 320 - 8)) + 'px'
                                }">
                                    ...
                                    <div class="px-4 pb-4 border-t border-gray-100 dark:border-gray-700">
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            ...
                                            <button
                                                wire:click="updateTaskDate('{{ today()->addDay()->format('Y-m-d') }}')"
                                                @click="dateOpen = false"
                                                class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
                                                Tomorrow
                                            </button>
    ...
    {{-- Custom CSS untuk Scrollbar dan Consistent Height --}}
    <style>
        ...
        /* Ensure consistent height untuk semua elemen interaktif */
        .h-8 {
