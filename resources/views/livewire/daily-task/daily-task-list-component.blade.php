            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <x-filament::button wire:click="resetFilters" color="gray" size="sm" 
                        icon="heroicon-o-arrow-path" outlined>
                            ...
                            <div class="bg-gradient-to-r from-gray-25 to-gray-50 px-6 py-4 border-l-4 border-l-primary-500"
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">{{ $tasks->count() }} {{ $tasks->count() === 1 ? 'task' : 'tasks' }}</p>
                                        </div>
                                    </div>
                                ...
                                <div x-show="!collapsed" x-transition:enter="transition ease-out duration-300"
                                    ...
                                    @foreach($tasks as $task)
                                        <div class="px-6 py-4 hover:bg-gradient-to-r hover:from-primary-50 hover:to-transparent transition-all duration-200 group border-l-4 border-l-transparent hover:border-l-primary-200 -mx-6 mb-2 last:mb-0 border-b border-gray-100 last:border-b-0">
                                            <livewire:daily-task.daily-task-item :task="$task" :key="'task-'.$task->id . time()" />
                                        </div>
                                    @endforeach
                                </div>
            ...
            <div class="lg:hidden space-y-3">
                @if($groupBy === 'none')
                    ...
                    @forelse($paginatedTasks as $task)
                        <div class="bg-white rounded-lg border-2 border-gray-200 shadow-sm hover:shadow-md hover:border-primary-300 transition-all duration-200">
                ...
                @else
                    {{-- Mobile Grouped View --}}
                    @forelse($groupedTasks as $groupName => $tasks)
                        <div class="bg-white rounded-lg border-2 border-gray-200 shadow-sm overflow-hidden hover:border-primary-300 transition-all duration-200" x-data="{ collapsed: false }">
                            {{-- Mobile Group Header --}}
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">{{ $tasks->count() }} {{ $tasks->count() === 1 ? 'task' : 'tasks' }}</p>
                                        </div>
                                    </div>
                            ...
                            <div x-show="!collapsed" x-transition:enter="transition ease-out duration-300"
                                ...
                                @foreach($tasks as $task)
                                    <div class="p-0 border-l-4 border-l-transparent hover:border-l-primary-300 hover:bg-gradient-to-r hover:from-primary-50 hover:to-transparent transition-all duration-200">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 lg:gap-6">
                @foreach(['pending', 'in_progress', 'completed', 'cancelled'] as $status)
                    @php
                        ...
                        $statusConfig = [
                            'pending' => ['color' => 'gray', 'bg' => 'bg-gray-50', 'border' => 'border-gray-200', 'icon' => 'heroicon-o-clock'],
                            'in_progress' => ['color' => 'warning', 'bg' => 'bg-yellow-50', 'border' => 'border-yellow-200', 'icon' => 'heroicon-o-play'],
                            'completed' => ['color' => 'success', 'bg' => 'bg-green-50', 'border' => 'border-green-200', 'icon' => 'heroicon-o-check-circle'],
                            'cancelled' => ['color' => 'danger', 'bg' => 'bg-red-50', 'border' => 'border-red-200', 'icon' => 'heroicon-o-x-circle']
                        ];
                    ...
                    <div class="bg-white rounded-lg lg:rounded-xl border-2 {{ $config['border'] }} overflow-hidden">
                        ...
                        <div class="p-3 lg:p-4 {{ $config['bg'] }} border-b {{ $config['border'] }}">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2 lg:gap-3">
                                    <div class="w-6 h-6 lg:w-8 lg:h-8 {{ $config['bg'] }} rounded-lg flex items-center justify-center border {{ $config['border'] }}">
                                        <x-dynamic-component :component="$config['icon']" class="w-3 h-3 lg:w-4 lg:h-4" />
                                    </div>
                                    <div>
                                        <h3 class="font-semibold text-gray-900 text-sm lg:text-base">{{ $statusOptions[$status] }}</h3>
                        ...
                        <div class="p-3 lg:p-4 space-y-3 min-h-[400px] lg:min-h-[500px] max-h-[600px] lg:max-h-[700px] overflow-y-auto">
                            @forelse($statusTasks as $task)
                                <div class="bg-white rounded-lg border border-gray-200 p-3 lg:p-4 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer group">
                                    {{-- Task Card Content --}}
                                    <div class="space-y-3">
                                        <div class="flex items-start justify-between">
                                            <h4 class="font-medium text-gray-900 text-sm line-clamp-2 group-hover:text-primary-600 transition-colors flex-1 pr-2">
                                                {{ $task->title }}
                                            </h4>
                                        ...
                                        <div class="flex items-center justify-between text-xs text-gray-500">
                                            <div class="flex items-center gap-1.5">
                                                ...
                                                <span class="{{ $task->task_date->isPast() && $task->status !== 'completed' ? 'text-red-600 font-medium' : '' }}">
                                        ...
                                        @if($task->assignedUsers && $task->assignedUsers->count() > 0)
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if($task->assignedUsers->count() === 1)
                                                    <span class="text-xs text-gray-600 truncate max-w-20 hidden sm:inline">{{ $task->assignedUsers->first()->name }}</span>
                                                @else
                                        ...
                                        <div class="lg:hidden pt-2 border-t border-gray-100">
                                            <div class="flex items-center justify-between">
                                                <button class="p-1.5 hover:bg-primary-100 text-primary-600 rounded transition-colors">
                            ...
                            @empty
                                <div class="text-center py-8">
                                    <div class="w-10 h-10 lg:w-12 lg:h-12 bg-gray-100 rounded-lg flex items-center justify-center mx-auto mb-3">
                                        <x-dynamic-component :component="$config['icon']" class="w-5 h-5 lg:w-6 lg:h-6 text-gray-400" />
                                    </div>
