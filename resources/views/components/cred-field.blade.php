...
<div class="space-y-1.5" x-data="{ show: false, copied: false, val: @js((string) $value) }">
    <div class="flex items-center justify-between">
        <label class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ $label }}</label>
        @if ($isPassword && (filled($value) || $isEdit))
            <button type="button" x-on:click="show = !show"
                    class="inline-flex items-center gap-1 text-[11px] font-medium text-gray-400 transition hover:text-gray-600 dark:hover:text-gray-200">
    ...
    @else
        ...
        @if (filled($value))
            <div class="group flex items-center gap-2">
                @if ($isPassword)
                    <span class="flex-1 truncate font-mono text-sm text-gray-900 dark:text-gray-100"
                ...
                <button type="button"
                        ...
                        class="shrink-0 rounded-md p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-gray-200"
